...
8. Log into the Enteprise Enterprise Manager UI using the bob token. Check your permissions again.


### Viewing Flow Logs

1. Run the following command to get the customer pod in the yaobank namespace.
...
```
2. Then run the following command and copy the IP address of the summary pod in the yaobank namespace.
...
```
3. Run the following command to exec into the customer pod. 

```
kubectl exec -it $(kubectl get pods -n yaobank -l app=customer -o name) -n yaobank -- sh
```

4. Use the IP address of the summary pod that we captured in the previous step and run the following curl command 5 times.
...
# exit
```
5. Run the following command and note the name of the node that the customer pod is running on. In this case the pod is running on ip-10-0-1-31.
...
6. ssh into worker2 and cd to the following directory. Make sure to ssh into the right node.
```
ssh worker2
cd /var/log/calico/flowlogs/
```
7. Then open the flows.log and look for customer pods. Ensure to run this command after few minutes you ran the curl command because it takes up to five minutes for felix to update the flow logs.
...
/customer
```
8. You should see an output similar to the following. Get yourself familiar with the output below. Some of the important fields are: "source_name_aggr", "source_namespace", "source_labels", "dest_name_aggr", "dest_namespace", "dest_labels", "dest_port", "num_flows", "num_flows_started","num_flows_completed",
...
9. Log into Calico Enterprise Manager UI using your token and browse to the "Flow Visualization" from the left navigation bar.
...
10. On the top right corner of the page, from the "All Namespaces" drop-down menu select "yaobank", from "All Names" drop-down menu select "customer" pod prefix, and from the "All Flows" drop-down menu select the "customer" pod prefix connection to "summary" pod prefix. Get yourself familiar with the flow visualizationa output. 
...
3. Log into Calico Enterprise Manager UI using your token and browse to the "Kibana" from the left navigation bar. A new browser page should open. Log into Kibana console using the provided credentials.
...
3. Click on the pencil icon for "allow-ingress-tcp" policy. Scroll to the bottom of the page and look at the "Change Log" info.
...
5. Then browse to the top right corner of edit policy page and click Enforce.
